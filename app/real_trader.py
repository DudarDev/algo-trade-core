...
class RealTrader:
    ...
    def buy(self, symbol, price, time):
        ...
        try:
            ...
            if fiat_balance < 10: # Мінімалка на Kraken ~$10
                self.logger.warning(f"⚠️ Недостатньо коштів ({fiat_balance} {currency}) для ордера.")
                return False
    ...
    def sell(self, symbol, price, time):
        ...
        try:
            ...
            # 2. Продаємо все, що є
            self.logger.info(f"📤 Відправляю ордер SELL: {coin_balance:.6f} {symbol}...")
            order = self.manager.exchange.create_order(symbol, 'market', 'sell', coin_balance)
