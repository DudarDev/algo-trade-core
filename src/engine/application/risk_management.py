...
# 2. ОЦЕЙ КЛАС БУВ ЗАГУБЛЕНИЙ! Відновлюємо його:
...
class RiskManager:
    ...
    def evaluate_trade(
        ...
        # Розрахунок та обмеження Stop Loss
        ...
        if actual_rr_ratio < self.config.min_risk_reward:
            logger.debug(f"🛑 Відхилено: R:R {actual_rr_ratio:.2f} < {self.config.min_risk_reward}")
            return None
